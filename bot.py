# bot.py
# Минимальный рабочий Telegram-бот на aiogram v3.7+
# - ReplyKeyboard (без callback'ов) => нет "вечной загрузки"
# - Свободный ввод всегда работает
# - Простая FSM: выбор темы -> ожидание деталей -> ответ -> сброс в меню
...
def topic_intro(topic: TopicType) -> str:
    ...
    if topic == "distracted":
        return (
            ...
            "2) Одно рабочее окно.\n"
            "3) 15–20 минут фокус, затем короткий перерыв.\n\n"
            "Напиши, чем занимаешься и что чаще всего отвлекает."
        )
    if topic == "overload":
        return (
            "Тема: «Перегруз».\n"
            "Снимаем давление:\n"
            "1) Выгрузи задачи списком (можно сюда).\n"
            "2) Отметим срочность/важность.\n"
...
def topic_response(topic: TopicType, user_text: str) -> str:
    ...
    if topic == "cant_start":
        return (
            "Действуем сейчас:\n"
            "• Сформулируй первый шаг на 10 минут, без перфекционизма.\n"
            f"• Учёл твоё описание: «{clip}».\n"
...
bot = Bot(
    token=TOKEN,
    default=DefaultBotProperties(parse_mode="HTML")  # ✅ правильный способ для aiogram 3.7+
)
